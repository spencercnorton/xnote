				...
				long  timeout_usecs)
{
	...
	*fd = socket (PF_INET, SOCK_STREAM, 0);
	...
	while (1) {
		...
		while (select (*fd + 1, NULL, &write_fds, NULL, &timeout) < 0) {
			if (errno != EINTR) {
		     ...
		     gpointer      user_data)
{
	...
	} while (info->bytes_written < total_bytes && status == G_IO_STATUS_NORMAL);
			      ...
			      va_list     args)
{
	...
	retval = dashboard_build_cluepacket_from_cluelist (frontend, focused, context, additive, clue_list);
			    ...
			    ...)
{
	...
	va_start (args, additive);

	retval = dashboard_build_cluepacket_v (frontend, focused, context, additive, args);
