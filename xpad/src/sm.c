...
static gboolean xpad_sm_cycle (gpointer data)
{
		...
		(GPOINTER_TO_INT (data)) ? NULL : &timeout) > 0)
	{
		...
		ice_conn = SmcGetIceConnection (xpad_sm_conn);
...
static void xpad_sm_start_interact_callback (SmcConn smc_conn, SmPointer client_data)
{
	blocking = FALSE;
}
...
gboolean xpad_sm_start_interact (gboolean error)
{
	...
	blocking = TRUE;
	status = SmcInteractRequest (xpad_sm_conn, 
		error ? SmDialogError : SmDialogNormal,
		xpad_sm_start_interact_callback, NULL);
...
void xpad_sm_stop_interact (gboolean stop_shutdown)
{
	...
	SmcInteractDone (xpad_sm_conn, stop_shutdown);
}

void xpad_sm_ice_connection_watch (IceConn ice_conn,
...
static void xpad_sm_set_properties (void)
{
	struct {
		SmPropValue clone[1];
...
void xpad_sm_init (void)
{
	...
	SmcCallbacks callbacks = {
		{xpad_sm_save_yourself, NULL},
		{xpad_sm_die, NULL},
		{xpad_sm_save_complete, NULL},
		{xpad_sm_shutdown_cancelled, NULL}};
	
	printf ("connecting with session id of %s\n", client_id);
	xpad_saving = FALSE;
	...
	xpad_sm_add_cycle_to_main_loop (NULL);
}

void xpad_sm_set_id (char *id)
{
	gboolean alread_set_up = client_id ? TRUE : FALSE;
...
void xpad_sm_shutdown (void)
{
	...
	SmcCloseConnection (xpad_sm_conn, 0, NULL);
}

static void xpad_sm_save_global (Bool fast)
{
	/* No need to do anything.  Currently, all xpad pads are always current with
	hard drive. */
}

static void xpad_sm_save_local (Bool fast)
{
	/* should also save cursor positions and open accessory windows */
	
	if (set_props)
	{
		xpad_sm_set_properties ();
		set_props = FALSE;
	}
}
	...
	int save_type, Bool shutdown, int interact_style, Bool fast)
{
	...
	{
	case SmSaveLocal:
		xpad_sm_save_local (fast);
		break;
	case SmSaveGlobal:
		xpad_sm_save_global (fast);
		break;
	case SmSaveBoth:
		xpad_sm_save_global (fast);
		xpad_sm_save_local (fast);
